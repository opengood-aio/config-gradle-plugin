...
class ExtensionsTest :
    FunSpec({
        ...
        test("determineFeatureEnabled returns enabled for feature for Groovy") {
            val project = createProject(ProjectConfig(languageType = LanguageType.GROOVY))

            val features =
                mapOf(
                    Feature.ASSERTJ to true,
                    Feature.JACKSON to true,
                    Feature.JACKSON_KOTLIN to false,
                    Feature.JUNIT_JUPITER to true,
                    Feature.KOTEST to false,
                    Feature.KOTEST_SPRING to false,
                    Feature.KOTLIN_COROUTINES to false,
                    Feature.LOMBOK to true,
                    Feature.MOCKITO to true,
                    Feature.MOCKK to false,
                    Feature.SPRING to true,
                    Feature.SPRING_MOCKK to false,
                )

            features.forEach { (feature, value) ->
                project.determineFeatureEnabled(feature) shouldBe value
            }
        }

        test("determineFeatureEnabled returns enabled for feature for Java") {
            val project = createProject(ProjectConfig(languageType = LanguageType.JAVA))
        ...
        test("determineFeatureEnabled returns enabled for feature for Kotlin") {
            val project = createProject(ProjectConfig(languageType = LanguageType.KOTLIN))

            val features =
                mapOf(
                    Feature.ASSERTJ to false,
                    Feature.JACKSON to true,
                    Feature.JACKSON_KOTLIN to true,
                    Feature.JUNIT_JUPITER to false,
                    Feature.KOTEST to true,
                    Feature.KOTEST_SPRING to true,
                    Feature.KOTLIN_COROUTINES to true,
                    Feature.LOMBOK to false,
                    Feature.MOCKITO to false,
                    Feature.MOCKK to true,
                    Feature.SPRING to true,
                    Feature.SPRING_MOCKK to true,
                )
        ...
        test("isSnapshotVersion returns true when project version property contains ${Versions.SNAPSHOT}") {
            val project = createProject(ProjectConfig(languageType = LanguageType.KOTLIN, version = "1.0.0-SNAPSHOT"))
